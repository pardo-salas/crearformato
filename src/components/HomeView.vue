...
const convertToJson = () => {
  ...
  for (let i = 0; i < valores.value.length; i++) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(valores.value[i].nombre.toUpperCase(), gethalf(doc.getStringUnitWidth(valores.value[i].nombre),10,docWidth-20,scaleFactor),55+altura);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.text(ocupacion.value,(doc.internal.pageSize.getWidth()/2)+2,64+altura)
    //Curp
    doc.setTextColor(0, 0, 0);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    ...
    for (let index = 0; index < 18; index++) {
      doc.text(valores.value[i].curp[index], x-3.8,65+altura);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(valores.value[i].puesto.toUpperCase(), 11,74+altura);  
    //Empresa
    doc.setTextColor(0, 0, 0);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(razon_social.value, 11,91+altura);
    ...
    for (let index = 0; index < 14; index++) {
      ...
      doc.text(rfc.value[index], xRFC-3.4,102+altura);
      xRFC+=5
    }
    //Nombre del Curso
    doc.setTextColor(0, 0, 0);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(valores.value[i].duracion, 30, 128+altura);
    ...
    for (let index = 0; index < 4; index++) {
      doc.text(valores.value[i].fecha_inicio.getFullYear().toString()[index], x+2.5, 128.5+altura);
      doc.text(valores.value[i].fecha_termino.getFullYear().toString()[index], xDFin+2.5, 128.5+altura);
    ...
    if((valores.value[i].fecha_inicio.getMonth()+1).toString().length == 1) {
      ...
      doc.text((valores.value[i].fecha_inicio.getMonth()+1).toString()[1], 121.5, 128.5+altura);
    }
    ...
    if((valores.value[i].fecha_termino.getDate()).toString().length == 1){
      ...
      doc.text((valores.value[i].fecha_termino.getDate()).toString(), 196.75, 128.5+altura);
    }else{
      doc.text((valores.value[i].fecha_termino.getDate()+1).toString()[0],189.25,128.5+altura)
      doc.text((valores.value[i].fecha_termino.getDate()+1).toString()[1],196, 128.5+altura);
    }
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(capacitador.value.toUpperCase(), 11, 148+altura);
    doc.text(acestps.value.toUpperCase(), 98, 148+altura);
    ...
    doc.setFontSize(6);
    doc.setFont('helvetica', 'bold')
    doc.text(instructor.value.toUpperCase(), gethalf(doc.getStringUnitWidth(instructor.value),6,45,scaleFactor)+20 ,190+altura);
    doc.line(20,192+altura,65,192+altura)
    doc.text(replegal.value.toUpperCase(), gethalf(doc.getStringUnitWidth(replegal.value),6,55,scaleFactor)+75 ,190+altura);
    doc.line(75,192+altura,130,192+altura)
    doc.text(pretrabajador.value.toUpperCase(), gethalf(doc.getStringUnitWidth(replegal.value),6,55,scaleFactor)+140 ,190+altura);
